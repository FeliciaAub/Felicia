...
export var py_metatype = {
  ...
  __new__: function(meta, name, bases, attribs) {
    ...
    for (let symbol of Object.getOwnPropertySymbols(attribs)) {
      let descrip =
        Object.getOwnPropertyDescriptor(attribs, symbol);
      Object.defineProperty(cls, symbol, descrip)
    }
    return cls
  }
...
export function __mergekwargtrans__(object0, object1) {
  ...
  for (var attrib in object1) result[attrib] =
    object1[attrib];
  return result
}
export function __mergefields__(targetClass, sourceClass) {
  ...
  if (sourceClass[fieldNames[0]])
    if (targetClass[fieldNames[0]])
      for (let fieldName of fieldNames) targetClass[fieldName] = new Set([...targetClass[fieldName], ...sourceClass[fieldName]]);
    else
      for (let fieldName of fieldNames) targetClass[fieldName] = new Set(sourceClass[fieldName])
}
...
export function __specialattrib__(attrib) {
  return attrib.startswith("__") && attrib.endswith("__") ||
    attrib == "constructor" || attrib.startswith("py_")
}
export function len(anObject) {
  ...
  for (var attr in anObject)
    if (!__specialattrib__(attr)) length++;
  return length
}
...
export function __t__(target) {
  return target === undefined || target === null ? false : ["boolean", "number"].indexOf(typeof target) >= 0 ? target : target.__bool__ instanceof Function ? target.__bool__() ? target : false : target.__len__ instanceof Function ? target.__len__() !== 0 ? target : false : target instanceof Function ? target : len(target) !== 0 ? target : false
}
...
export function callable(anObject) {
  return anObject && typeof anObject ==
    "object" && "__call__" in anObject ? true : typeof anObject === "function"
}
...
export function round(number, ndigits) {
  ...
  if (rounded - number == .5 && rounded % 2) rounded -= 1;
...
export function zip() {
  ...
  var shortest = args.length == 0 ? [] : args.reduce(function(array0,
    array1) {
...
export function copy(anObject) {
  if (anObject == null || typeof anObject == "object") return anObject;
    ...
    for (var attrib in obj)
      if (anObject.hasOwnProperty(attrib)) result[attrib] = anObject[attrib];
    return result
  }
}
export function deepcopy(anObject) {
  if (anObject == null || typeof anObject == "object") return anObject;
    ...
    var result = {};
    for (var attrib in obj)
      if (anObject.hasOwnProperty(attrib)) result[attrib] = deepcopy(anObject[attrib]);
    return result
  }
}
...
Array.prototype.__getslice__ = function(start, stop, step) {
  ...
  else if (stop < 0) stop = this.length +
    stop;
...
Array.prototype.__setslice__ = function(start, stop, step, source) {
  ...
  if (step == null) Array.prototype.splice.apply(this, [start, stop - start].concat(source));
  else {
    let sourceIndex = 0;
    for (let targetIndex =
        start; targetIndex < stop; targetIndex += step) this[targetIndex] = source[sourceIndex++]
  }
};
...
export function set(iterable) {
  let instance = [];
  if (iterable)
    for (let index = 0; index < iterable.length; index++) instance.add(iterable[index]);
...
Array.prototype.union = function(other) {
  ...
  for (let i = 0; i < other.length; i++)
    if (result.__bindexOf__(other[i]) == -1) result.push(other[i]);
  return result
};
Array.prototype.intersection = function(other) {
  ...
  let result = set();
  for (let i = 0; i < other.length; i++)
    if (this.__bindexOf__(other[i]) != -1) result.push(other[i]);
  return result
};
Array.prototype.difference = function(other) {
  ...
  for (let i = 0; i < this.length; i++)
    if (sother.__bindexOf__(this[i]) == -1) result.push(this[i]);
  return result
};
...
Array.prototype.py_update = function() {
  ...
  for (let i = 0; i < updated.length; i++)
    if (updated[i] != updated[i - 1]) this.push(updated[i])
};
...
export function bytearray(bytable, encoding) {
  if (bytable ==
    undefined) return new Uint8Array(0);
  else {
...
String.prototype.endswith = function(suffix) {
  if (suffix instanceof Array)
    for (var i = 0; i < suffix.length; i++) {
      if (this.slice(-suffix[i].length) ==
        suffix[i]) return true
...
String.prototype.__getslice__ = function(start, stop, step) {
  ...
  if (step == 1) result = this.substring(start, stop);
    ...
    for (var index = start; index < stop; index += step) result = result.concat(this.charAt(index));
  return result
};
__setproperty__(String.prototype,
  "format", {
    get: function() {
...
String.prototype.py_split = function(sep, maxsplit) {
  ...
  if (maxsplit == undefined || maxsplit ==
    -1) return stripped.split(sep);
  else {
    var result = stripped.split(sep);
    if (maxsplit < result.length) return result.slice(0, maxsplit).concat([result.slice(maxsplit).join(sep)]);
    else return result
  }
};
...
function __keys__() {
  ...
  for (var attrib in this)
    if (!__specialattrib__(attrib)) keys.push(attrib);
  return keys
}
...
function __items__() {
  var items = [];
  for (var attrib in this)
    if (!__specialattrib__(attrib)) items.push([attrib, this[attrib]]);
  return items
}
...
function __pop__(aKey, aDefault) {
  ...
  if (result != undefined) {
    delete this[aKey];
    return result
  } else if (aDefault === undefined) throw KeyError(aKey, new Error);
  return aDefault
}
...
function __values__() {
  var values = [];
  for (var attrib in this)
    if (!__specialattrib__(attrib)) values.push(this[attrib]);
  return values
}
    ...
    if (objectOrPairs)
      for (var index = 0; index < objectOrPairs.length; index++) {
        ...
        if (!(objectOrPairs instanceof Array) && objectOrPairs instanceof Object)
          if (!isinstance(objectOrPairs, dict)) val =
            dict(val);
        instance[key] = val
      }
  } else if (isinstance(objectOrPairs, dict)) {
...
export function __truediv__(a, b) {
  ...
  else if (typeof b == "object" && "__rdiv__" in
    b) return b.__rdiv__(a);
  else return a / b
}
...
export function __rshift__(a, b) {
  ...
  else if (typeof b == "object" && "__rrshift__" in
    b) return b.__rrshift__(a);
  else return a >> b
}
...
export function __ijsmod__(a, b) {
  if (typeof a == "object" &&
    "__imod__" in a) return a.__ismod__(b);
...
export function __ilshift__(a, b) {
  if (typeof a == "object" &&
    "__ilshift__" in a) return a.__ilshift__(b);
...
export function __ior__(a, b) {
  if (typeof a ==
    "object" && "__ior__" in a) return a.__ior__(b);
...
export function __setitem__(container, key, value) {
  if (typeof container == "object" && "__setitem__" in
    container) container.__setitem__(key, value);
...
export function __getslice__(container, lower, upper, step) {
  if (typeof container == "object" && "__getitem__" in container) return container.__getitem__([lower, upper, step]);
  else return container.__getslice__(lower, upper, step)
}
export function __setslice__(container, lower, upper, step, value) {
  if (typeof container == "object" && "__setitem__" in
    container) container.__setitem__([lower, upper, step], value);
...
export var filter = function(func, iterable) {
  ...
  return function() {
    ...
    for (var item of iterable)
      if (func(item)) __accu0__.append(item);
    return __accu0__
  }()
};
